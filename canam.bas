...
180 ' POKE SAM, WILDMAN WILLY, AND HOTSHOT HARRY
190 '
200 ' INSTRUCTIONS--
...
730 LET K9=6                            'TWICE NUMBER OF KNOWN LANGUAGES
...
1540      LET Y=FNA(M$(1),200,1,3/10,65) 'STRAIGHT A, 200 MAX., 3/10 MILE LONG
1550      LET Y=FNA(M$(3),125,1,1/10,49) 'CURVE 1, 125 MAX., 1/10 MILE LONG
1560      LET Y=FNA(M$(1),200,2,13/20,66) 'STRAIGHT B, 200 MAX., 13/20 MILE LONG
1570      LET Y=FNA(M$(3),125,1,1/10,50) 'CURVE 2, 125 MAX., 1/10 MILE LONG
1580      LET Y=FNA(M$(1),200,1,1/5,67)  'STRAIGHT C, 200 MAX., 1/5 MILE LONG
...
1690      LET Y=FNA(M$(1),200,2,7/10,72) 'STRAIGHT H, 200 MAX., 7/10 MILE LONG
...
2020 PRINT O$((N-F2)/(F2+1));"NICE RACE, MR.";N
...
2100 FOR Z=0 TO 10                      'RESET FOR NEXT GAME
2110      LET T(Z)=D(Z)=0
2120 NEXT Z
...
2320                                    'ASCI(LETTER FOLLOWING KIND OF TRACK)
...
2490      GOTO 2780
2500      LET Y4=H(G)                   'RATS.  HE DIDN'T WIPE
...
2620           PRINT "YOU JUST PASSED ";O$(X4)
2630           GOTO 2650
2640           PRINT O$(X4);"JUST PASSED YOU"
2650      NEXT X4
...
2760           PRINT O$(X4);"YOU JUST PASSED GUY";F2*(G+1)+G
2770      NEXT X4
...
2990 IF A(0)<.1 THEN 3060               'IS IT RAINING?
...
3190      PRINT O$(X);"YOU ARE IN THE PITS FOR:";L(X);"SECONDS."
3200 NEXT X
3210 RETURN                             'WE WUZ HERE (AND LEFT!)
3220 REM  INPUT
3230 MAT S=ZER                          'RESET SPEED ARRAY
3240 IF O9>0 THEN 3270                  'PRINT OUT SOMETHING LIKE 'STRAIGHT A'
3250 PRINT A$;" ";CHR$(D);
3260 GOTO 3280
...
3650 GOTO 3340
3660 IF S=>20 THEN 3690                 'WHERE DOES HE THINK HE IS?  THE LONG ISLAND EXPRESSWAY?
...
3810      LET D(F0)=1
3820      PRINT O$(10);O$(F0);" JUST WIPED " 'AND INFORM US IF THEY WIPE
3830      LET F5=F5+1
3840      LET E(F5)=D
...
3870 NEXT F0
3880 GOTO 3980
3890 FOR X2=0 TO 09                     'NOW HANDLE DRAFTERS
3900      IF D(X2)=1 THEN 3970
3910      IF S(X2)=0 THEN 3970
3920      IF H(X2)=0 THEN 3950
...
3990 REM PLACING
4000 PRINT O$(A)                        'TELL ME HOW I'M DOING
...
4080      PRINT O$(G);" IS";            'I AM RACING AGAINST AUTO-PILOTS
4090      GOTO 4110
4100      PRINT "GUY";F2*(G+1)+G;"IS";  'I AM RACING AGAINST FRIENDS(?)
4110      IF D(G)<>1 THEN 4140
4120      PRINT " OUT OF THE RACE."     'IF COMPETITOR HAS MET HIS MAKER, SAY SO
4130      GOTO 4200
4140      ON SGN(T(A)-T(G))+2 GOTO 4150,4170,4190 'BUSINESS PART
4150      PRINT T(A)-T(G);"SECONDS BEHIND YOU" 'GOOD NEWS
4160      GOTO 4200
